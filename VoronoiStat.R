...
# -> 1. File Read
file_path = "data/VoronoiStat/VoronoiStat.txt"
file_format = "txt"
# "xlsx", "xls", "txt", "csv"
...
# -> 2. Data Operation
# set.seed(123)
# Xvalue <- sample(1:400, size = 100)
# Yvalue <- sample(1:400, size = 100)
# Distance <- sqrt((Xvalue - 200) ^ 2 + (Yvalue - 200) ^ 2)
# 
# df <- data.frame(Xvalue, Yvalue, Distance = Distance)
# write.table(df, "VoronoiStat.txt", quote = F, sep = "\t", row.names = F)
# <- 2. Data Operation
...
naDeal <- "NA_Retain"
# ChoiceBox: "NA_Retain", "NA_Remove"
if (naDeal == "NA_Retain") {
    na_rm <- FALSE
} else if (naDeal == "NA_Remove") {
    na_rm <- TRUE
}
...
p <- ggplot(data, 
    ...
    geom_voronoi(na.rm = na_rm,
                 alpha = voronoiAreaAlpha
                 ) +
    stat_voronoi(geom = "path",
                 position = "identity", 
                 na.rm = na_rm,
                 linetype = voronoiLineType,
                 lwd = voronoiLineWidth
                 ) +
    ...
    scale_color_gradient(
        ...
        space = "Lab",
        na.value = naColor,
        guide = "colourbar", # "colourbar", "legend"
